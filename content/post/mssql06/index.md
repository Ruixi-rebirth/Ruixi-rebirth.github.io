...
- [CREATE](http://ruixi.me/p/mssql06_1/#create) ：此命令用于创建数据库或其对象（如表、索引、函数、视图、存储过程和触发器）。
- [DROP](http://ruixi.me/p/mssql06_1/#drop) ：此命令用于从数据库中删除对象。
- [ALTER](http://ruixi.me/p/mssql06_1/#alter) ： 这用于更改数据库的结构。
- [TRUNCATE](http://ruixi.me/p/mssql06_1/#truncate) ： 这用于从表中删除所有记录，包括删除为记录分配的所有空间。
- COMMENT ：用于向数据字典添加注释。
...
- INSERT ：用于向表中插入数据。
- UPDATE ： 它用于更新表中的现有数据。
- [DELETE](http://ruixi.me/p/mssql06_1/#delete) ：用于从数据库表中删除记录。
- LOCK： 表控制并发。
